    ...
    */
    ...
    SongCollection::SongCollection(const char* file){
        ...
        if(sourceFile){
            ...
            std::string temp_artist, temp_title, temp_album, line;
            double temp_price;
            int temp_year, temp_songLength;
            size_t pos,len;
            while (sourceFile){
                pos=0;
                len=25;
                ...
                try{
                    ...
                    temp_year=std::stoi(trim(line.substr(pos,len)));
                    pos+=len;
    ...
    */
    std::ostream& operator<<(std::ostream& out, const Song& theSong){
        out<<"| "<<std::setw(20)<<std::left<<theSong.title<<" | "
            ...
            <<std::right<<(theSong.songLength)/60<<":"
            <<std::setfill('0')<<std::setw(2)<<theSong.songLength%60<<" | "
            <<std::setfill(' ')
            <<std::right<<theSong.price<<" | "<<std::endl;
        return out;
    }
