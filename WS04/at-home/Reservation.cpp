...
namespace sdds{
    ...
    */
    void Reservation::init(const Reservation& src){
        ...
        if(src.reservationID!=nullptr){
			...
			int count = strlen(src.reservationID);
            //dynamically allocate space
            reservationID=new char[count+1];
            //copy each element
			strncpy(reservationID, src.reservationID, count+1);
        }
        else{
            reservationID=nullptr;
        }
    ...
    /*
     move constructor
    */
    Reservation::Reservation(Reservation&& src){
        //todo
        *this=std::move(src);
    }
    ...
    */
    Reservation& Reservation::operator=(Reservation&& rhs){
        std::cout<<"came here";
        if(this!=&rhs){
            delete [] reservationID;
            totalGuest=rhs.totalGuest;
            day=rhs.day;
            hour=rhs.hour;
            name=rhs.name;
            email=rhs.email;
            reservationID=rhs.reservationID;
            rhs.reservationID=nullptr;
        }
        return *this;
    }
   ...
   */
    ...
    */
    ...
    //overloaded == operator
    //this operator returns true if both operand are same
    //otherwise false
    bool Reservation::operator==(const Reservation& rhs)const{
        bool match1, match2;
        match1=(totalGuest==rhs.totalGuest && day==rhs.day && hour==rhs.hour && name==rhs.name && email==rhs.email);
        match2=(strcmp(reservationID, rhs.reservationID)==0);
